...
def display_dashboard():
    ...
    # Create a DataFrame-like structure for Streamlit
    dashboard_data = []
    ...
    # Display as a Streamlit Data Table with Styling
    st.dataframe(
        dashboard_data,
        column_config={
            "Asset": "Trading Pair",
            ...
            "% Change (24h)": st.column_config.NumberColumn(
                "% Change (24h)",
                format="%.2f %%",
                help="24-hour price percentage change"
            ),
            **{
                f"Vol Δ ({w}m)": st.column_config.NumberColumn(
                    f"Vol Δ ({w}m)",
                    format="%.2f %%",
                    help=f"Volume change over last {w} minutes"
                )
                for w in WINDOW
            }
        },
